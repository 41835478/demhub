...
<div id="welcome_division" class="row">
	@foreach($xmlcategories as $category)
		<a href="{{url('division', array('id' => $category->id))}}">
			<div id="division_{{$category->id}}" style="background: url('images/backgrounds/divisions/{{$category->id}}.jpg') no-repeat center fixed;
				border-top:7px solid #{{$category->bg_color}}; border-bottom: 7px solid #{{$category->bg_color}}" class="col-md-2">
